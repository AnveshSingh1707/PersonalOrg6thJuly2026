({
    ...
    selectItemHelper : function(component, event) {
        ...
        var tempNumberOfSelections = component.get('v.numberOfSelections');
        console.log('tempNumberOfSelections-->',tempNumberOfSelections);
        var options = component.get('v.options');
        var multiSelect = component.get('v.multiSelect');
        ...
        options.forEach( function(element, index) {
            if(element.value === event.currentTarget.id) {
                if(multiSelect) {
                    ...
                    var finalCondition = element.selected ? true : valuesLength <= tempNumberOfSelections;
                    if(finalCondition) {
                        if(values.includes(element.value)) {
