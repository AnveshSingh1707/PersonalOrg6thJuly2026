trigger AttachmentTrigger on Attachment (before insert,before update,before delete,after insert, after update, after delete, after undelete) {
    AttachmentTriggerHandler handler = new AttachmentTriggerHandler(trigger.new,trigger.newMap,trigger.old,trigger.oldMap,trigger.isInsert,trigger.isUpdate,trigger.isDelete,trigger.isUndelete);
    if(Trigger.isAfter){
        if(trigger.isInsert){
